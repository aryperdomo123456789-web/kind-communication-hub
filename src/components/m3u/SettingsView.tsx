  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="bg-[#1a1a1a] p-5 md:p-8 rounded-2xl border border-neutral-800 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="space-y-2">
            <label className="text-xs text-neutral-500 uppercase font-bold px-1">
              Nome de Exibição
            </label>
            <input
              value={newName}
      ...
      <div className="space-y-4">
        <h3 className="text-sm text-neutral-500 uppercase font-bold px-2 tracking-widest">
          Suas Listas Configuradas
        </h3>
        <div className="grid gap-3">
          {lists.map((list) => (
            <div
              key={list.url}
              className={`group flex flex-col md:flex-row md:items-center justify-between p-4 md:p-5 rounded-2xl border transition-all gap-4 ${activeUrl === list.url ? "bg-blue-600/5 border-blue-600/50 shadow-inner shadow-blue-600/5" : "bg-[#141414] border-neutral-800 hover:border-neutral-700"}`}
            >
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center ${activeUrl === list.url ? "bg-blue-600 text-white" : "bg-neutral-800 text-neutral-400"}`}
                >
                  <List size={24} />
                </div>
                <div>

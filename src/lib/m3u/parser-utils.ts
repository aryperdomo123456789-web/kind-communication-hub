...
export function detectType(urlLower: string, rawName: string): "movie" | "series" | "live" {
  ...
  if (urlLower.includes("/live/") || urlLower.endsWith(".ts") || urlLower.endsWith(".m3u8")) return "live";
  
  if (rawName.includes("S0") || rawName.includes("E0") || /\d+x\d+/.test(rawName)) return "series";
  if (urlLower.includes("vod") || urlLower.endsWith(".mp4") || urlLower.endsWith(".mkv")) return "movie";
